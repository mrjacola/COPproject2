...
if __name__ == "__main__":
    ...
    num_steps = 20000
    therm_steps = 5000
    alphas = np.linspace(0.1, 0.25, num=20)
    ...
    for a in alphas:
        ...
        for j in range(0, 4999):  # Resampling 5000 times
            sample[j] = np.mean(np.random.choice(energy_est.flatten(),num_steps-therm_steps)) 
        energies_error[i] = np.std(sample)
